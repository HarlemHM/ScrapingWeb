    ...
    try:
        ...
        for selector in modal_selectors:
            ...
            if modal.count() > 0 and modal.first.is_visible():
                print(f"­ƒö╣ Modal detectado con selector {selector}, cerrando...")
                ...
                for close_selector in close_selectors:
                    ...
                    if close_btn.count() > 0 and close_btn.first.is_visible():
                        try:
                            ...
                            modal_closed = True
                            print("Ô£à Modal cerrado con botón")
                            break
                        except:
                            continue
                ...
                if not modal_closed:
                    ...
                    try:
                        ...
                        print("Ô£à Modal cerrado con Escape")
                    except:
                        pass
        ...
        print(f"ÔÜá´©Å Error cerrando modal: {e}")
...
def guardar_progreso(reseñas, pagina_actual):
    ...
    print(f"­ƒÆ¥ Progreso guardado: {len(reseñas)} reseñas en {filename}")


def scroll_para_cargar_todos(page):
    """Hace scroll incremental hasta que no aparezcan m├ís listados nuevos."""
    prev_count = -1
    while True:
...
def scroll_y_contar_reseñas_mejorado(page):
    """
    Función mejorada que hace scroll Y cuenta las reseñas usando m├║ltiples selectores
    para asegurar que realmente estí cargando todo el contenido.
    """
    print("­ƒöä Iniciando scroll inteligente para cargar reseñas...")

    # Primero, obtener el n├║mero total de reseñas esperadas
    total_esperado = 0
    try:
        ...
        if total_element.count() > 0:
            ...
            if match:
                ...
                print(f"­ƒôè Total de reseñas esperadas: {total_esperado}")
    except:
        pass

    # Selectores m├ís robustos para contar reseñas
    count_selectors = [
    ...
    max_no_change = 8  # Aumentado para ser m├ís persistente

    while scroll_count < max_scrolls and no_change_cycles < max_no_change:
        # Contar con todos los selectores y tomar el m├íximo
        current_max_count = 0
        ...
        print(f"­ƒôè Scroll {scroll_count + 1}: {current_max_count} reseñas detectadas (m├íximo)")

        # Scroll m├ís agresivo y variado
        try:
            ...
            if current_max_count > prev_max_count:
                time.sleep(4)  # M├ís tiempo cuando hay cambios
            else:
                time.sleep(2)  # Menos tiempo cuando no hay cambios

        except Exception as e:
            print(f"ÔÜá´©Å Error en scroll {scroll_count + 1}: {e}")
            time.sleep(1)

        # Verificar cambios
        if current_max_count > prev_max_count:
            no_change_cycles = 0  # Reset contador si hay cambios
            print(f"Ô£à Se cargaron {current_max_count - prev_max_count} reseñas adicionales")
        else:
            no_change_cycles += 1
            print(f"ÔÅ│ Sin cambios ({no_change_cycles}/{max_no_change})")

        # Verificar si hemos alcanzado el total esperado
        if total_esperado > 0 and current_max_count >= total_esperado * 0.9:  # 90% del total
            print(f"Ô£à Se alcanzó el 90% del total esperado ({total_esperado})")
            break

        prev_max_count = current_max_count
        scroll_count += 1

        # Cada 15 scrolls, hacer una pausa m├ís larga y limpiar memoria
        if scroll_count % 15 == 0:
            print(f"­ƒöä Pausa de limpieza despuís de {scroll_count} scrolls...")
            time.sleep(3)
    ...
    print(f"Ô£à Scroll completado: {final_count} reseñas detectadas despuís de {scroll_count} scrolls")
    return final_count


def extraer_reseñas_individuales_mejorado(page):
    """
    Función mejorada que extrae reseñas usando m├║ltiples estrategias
    y maneja mejor el lazy loading.
    """
    print("­ƒôØ Iniciando extracción mejorada de reseñas...")
    reseñas = []

    # Hacer un ├║ltimo scroll para asegurar que todo estí cargado
    try:
        page.evaluate("""
            let modal = document.querySelector('[role="dialog"]') || 
                       document.querySelector('[aria-modal="true"]');
    ...
    # Selectores m├║ltiples para encontrar elementos de reseñas
    ...
    for selector in review_selectors_prioritarios:
        try:
            ...
            print(f"­ƒöì Probando selector prioritario '{selector}': {count} elementos")

            if count > elements_count:
                review_elements = elements
                elements_count = count
                print(f"Ô£à Mejor selector hasta ahora: '{selector}' con {count} elementos")
        except Exception as e:
            print(f"ÔÜá´©Å Error con selector '{selector}': {e}")
            continue

    # Si no encontramos suficientes, probar selectores alternativos
    if elements_count < 10:  # Si hay menos de 10, probablemente algo est├í mal
        print("­ƒöä Probando selectores alternativos...")
        for selector in review_selectors_alternativos:
            try:
                elements = page.locator(selector)
                count = elements.count()
                print(f"­ƒöì Probando selector alternativo '{selector}': {count} elementos")

                if count > elements_count:
                    review_elements = elements
                    elements_count = count
                    print(f"Ô£à Mejor selector alternativo: '{selector}' con {count} elementos")
            except Exception as e:
                print(f"ÔÜá´©Å Error con selector alternativo '{selector}': {e}")
                continue

    if not review_elements or elements_count == 0:
        print("ÔØî No se encontraron elementos de reseñas")
        return []

    print(f"­ƒôè Total de elementos a procesar: {elements_count}")
    ...
    for batch in range(total_batches):
        ...
        print(f"­ƒöä Procesando lote {batch + 1}/{total_batches} (elementos {start_idx + 1}-{end_idx})...")

        for i in range(start_idx, end_idx):
            try:
                review = review_elements.nth(i)

                # Verificar que el elemento sea visible y v├ílido
                if not review.is_visible():
                    continue

                # Hacer scroll suave hacia el elemento para asegurar que estí completamente cargado
                try:
                ...
                # Extraer datos con timeouts m├ís cortos para mejor rendimiento
                reseña_data = extraer_datos_reseña(review, page, i + 1)

                if reseña_data:
                    reseñas.append(reseña_data)
                    if (i + 1) % 10 == 0:  # Progreso cada 10 reseñas
                        print(f"­ƒôè Progreso: {len(reseñas)} reseñas v├ílidas de {i + 1} procesadas")

            except Exception as e:
                print(f"ÔØî Error procesando elemento {i + 1}: {e}")
                continue
    ...
    print(f"­ƒÄë Extracción completada: {len(reseñas)} reseñas v├ílidas de {elements_count} elementos procesados")
    return reseñas
...
def extraer_datos_reseña(review, page, indice):
    ...
    try:
        ...
        fecha = "N/A"
        try:
            # Selectores especáficos basados en el HTML proporcionado
            fecha_selectors = [
                "div.s78n3tv",  # Contenedor principal de fecha
                "div[class*='s78n3tv']",  # Variante del selector
                "div:has-text('agosto')",  # Buscar por mes especáfico
            ...
            # Patrones de fecha m├ís amplios
            patrones_fecha = [
                ...
                r'hace\s+un?\s+(dáa|semana|mes|año)',
                r'hace\s+\d+\s+(dáas|semanas|meses|años)',
                    ...
                    try:
                        ...
                        if (20 <= len(text) <= 1000 and
                                not any(word in text.lower() for word in
                                        ["estrellas", "años en", "año en", "estadáa"] + meses)):
        ...
        try:
            # Selectores especáficos basados en el HTML proporcionado
            ubicacion_selectors = [
                "div.s15w4qkt",  # Selector especáfico de ubicación
                "div[class*='s15w4qkt']",  # Variante del selector
                "div:has-text('Colombia')",  # Buscar por paás
                "div:has-text('Cali')",  # Buscar por ciudad especáfica
                "div:has-text('Bogot├í')",
                "div:has-text('Medellán')",
                "div:has-text('Barranquilla')",
                "div:has-text('Cartagena')"
            ]
            
            ciudades_colombia = [
                "Colombia", "Bogot├í", "Medellán", "Cali", "Barranquilla", 
                "Cartagena", "Bucaramanga", "Pereira", "Santa Marta",
                "Manizales", "Ibaguí", "Pasto", "Neiva", "Villavicencio"
            ]
                ...
                try:
                    ...
                    for elem in ubicacion_elements.all()[:5]:  # Aumentar b├║squeda
                        ...
                        if (any(ciudad in text for ciudad in ciudades_colombia) and
                            ...
                            "estadáa" not in text and
                            len(text) <= 100):  # Aumentar lámite
                            ubicacion = text
                            break
                    if ubicacion != "N/A":
                        break
        ...
        # Extraer tipo de estadáa
        tipo_estadia = "N/A"
        try:
            # Selectores especáficos para tipo de estadáa
            estadia_selectors = [
                "div.rdyyd4g",  # Selector especáfico de tipo de estadáa
                "div[class*='rdyyd4g']",  # Variante del selector
                "div:has-text('Estadáa de varias noches')",
                "div:has-text('Estadáa de una noche')",
                "div:has-text('Estadáa de')",
                "div:has-text('varias noches')",
                "div:has-text('una noche')"
            ]
            
            tipos_estadia = [
                "Estadáa de varias noches",
                "Estadáa de una noche", 
                "varias noches",
                "una noche"
            ]
            ...
            for selector in estadia_selectors:
                try:
                    ...
                    for elem in estadia_elements.all()[:3]:  # Limitar b├║squeda
                        text = elem.inner_text(timeout=800).strip()
                        # Buscar texto que contenga tipos de estadáa
        ...
        # Solo devolver reseñas con información mánima v├ílida
        ...
        print(f"ÔØî Error extrayendo datos de reseña {indice}: {e}")
        return None


def extraer_reseñas_alojamiento(page):
    """Extrae todas las reseñas de un alojamiento especáfico - VERSI├ôN MEJORADA"""
    todas_reseñas = []

    try:
        # PASO 1: Hacer scroll hacia abajo para cargar la sección de reseñas
        print("­ƒöì Buscando sección de reseñas...")
        page.evaluate("window.scrollTo(0, document.body.scrollHeight / 2)")
        time.sleep(2)
        
        # PASO 2: Cerrar cualquier modal que pueda estar abierto
        print("­ƒö╣ Cerrando modales antes de buscar botón de reseñas...")
        cerrar_modal(page)
        time.sleep(1)
        
        # PASO 3: Buscar el botón "Mostrar todas las reseñas" con m├║ltiples estrategias
        ...
        print("­ƒöì Buscando botón de reseñas con selectores especáficos...")
            ...
            try:
                ...
                if btn.count() > 0:
                    ...
                    if first_btn.is_visible():
                        ...
                        print(f"Ô£à Botón encontrado con selector: {selector}")
                        break
            except Exception as e:
                print(f"ÔÜá´©Å Error con selector {selector}: {e}")
                continue

        # PASO 4: Si no encuentra con selectores especáficos, buscar por texto
        if not show_reviews_btn:
            print("­ƒöì Buscando botón por texto 'reseñas'...")
            try:
                # Buscar botones que contengan "reseñas" pero usar .first para evitar m├║ltiples matches
                btn = page.locator("button:has-text('reseñas')").first
                if btn.count() > 0 and btn.is_visible():
                    show_reviews_btn = btn
                    print("Ô£à Botón encontrado con texto 'reseñas'")
            except Exception as e:
                print(f"ÔÜá´©Å Error buscando por texto: {e}")

        # PASO 5: Si a├║n no encuentra, buscar con JavaScript
        if not show_reviews_btn:
            print("­ƒöì Buscando botón con JavaScript...")
            try:
                ...
                if js_result.get('found'):
                    print(f"Ô£à Botón encontrado con JS: {js_result['selector']} - Texto: '{js_result['text']}'")
                ...
                print(f"ÔÜá´©Å B├║squeda con JavaScript falló: {js_error}")

        # PASO 6: Procesar seg├║n si se encontró el botón o no
        if show_reviews_btn:
            if show_reviews_btn != "js_clicked":
                print("­ƒö╣ Haciendo clic en el botón de reseñas...")
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            click_successful = True
                            print("Ô£à Clic realizado con JavaScript")
                        except:
                            try:
                                show_reviews_btn.press("Enter")
                                click_successful = True
                                print("Ô£à Clic realizado con Enter")
                            except:
                                pass

                    if not click_successful:
                        print("ÔØî No se pudo hacer clic en el botón de reseñas")
                        return []

                except Exception as e:
                    print(f"ÔØî Error haciendo clic en botón: {e}")
                    return []

            # Esperar a que aparezca el modal
            print("­ƒöì Esperando a que aparezca el modal de reseñas...")
            ...
            for selector in modal_selectors:
                try:
                    ...
                    modal_found = True
                    print(f"Ô£à Modal encontrado con selector: {selector}")
                    break
                except:
                    continue

            if not modal_found:
                print("ÔÜá´©Å No se encontró el modal, intentando extraer de la p├ígina principal...")
                return extraer_reseñas_individuales_mejorado(page)

            time.sleep(2)

            # AQUí EST├ü LA MEJORA PRINCIPAL: Scroll inteligente
            total_detected = scroll_y_contar_reseñas_mejorado(page)

            # Extraer reseñas con el mítodo mejorado
            print("­ƒôØ Iniciando extracción con mítodo mejorado...")
            ...
            print("ÔÜá´©Å No se encontró el botón de reseñas, extrayendo de la p├ígina principal...")
            todas_reseñas = extraer_reseñas_individuales_mejorado(page)

    except Exception as e:
        print(f"ÔØî Error general al extraer reseñas: {e}")
...
with sync_playwright() as p:
    ...
    while True:  # Sin lámite de p├íginas
        print(f"\n­ƒôä Procesando p├ígina {pagina}...")

        try:
            page.wait_for_selector("a[href*='/rooms/']", timeout=15000)
        except:
            print("ÔÜá´©Å No se encontraron alojamientos en esta p├ígina")
            break
        ...
        for i in range(links.count()):  # Sin lámite de alojamientos
        ...
        print(f"Se encontraron {len(hrefs)} alojamientos ├║nicos en esta p├ígina.")
        ...
        for idx, path in enumerate(hrefs_a_procesar, start=1):
            ...
            print(f"\n­ƒÅá Procesando alojamiento {idx}/{len(hrefs_a_procesar)} de la p├ígina {pagina}")
            print(f"­ƒöù URL: {url_completa}")

            try:
                cerrar_modal(page)
                esperar_sin_overlays(page)

                # M├║ltiples intentos de navegación
                ...
                for attempt in range(3):
                    try:
                        ...
                        print(f"ÔÜá´©Å Intento {attempt + 1} de navegación falló: {nav_error}")
                        if attempt < 2:  # Si no es el ├║ltimo intento
                            time.sleep(2)
                        continue

                if not navigation_successful:
                    print(f"ÔØî No se pudo navegar al alojamiento despuís de 3 intentos")
                    continue

                page.wait_for_selector("h1", timeout=15000)

                # Cerrar modales despuís de cargar la p├ígina
                time.sleep(1)
                cerrar_modal(page)
                ...
                print(f"­ƒôï Tátulo: {titulo_alojamiento}")

                # Extraer reseñas con mítodo mejorado
                ...
                print(f"Ô£à Extraádas {len(reseñas_alojamiento)} reseñas del alojamiento")
                print(f"­ƒôè Total acumulado: {len(todas_las_reseñas)} reseñas")

                # Guardar progreso cada 5 alojamientos (m├ís frecuente)
                if idx % 5 == 0:
                    guardar_progreso(todas_las_reseñas, pagina)
                    print(f"­ƒÆ¥ Progreso guardado autom├íticamente - Total: {len(todas_las_reseñas)} reseñas")
                ...
                print(f"ÔØî Error procesando alojamiento {idx}: {e}")
                continue

        # Ir a siguiente p├ígina - VERSI├ôN MEJORADA
        try:
            print(f"­ƒöä Intentando ir a la p├ígina {pagina + 1}...")
            
            # Scroll hacia abajo para asegurar que el botón estí visible
            page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
            page.wait_for_timeout(2000)
            
            # ESTRATEGIA 1: Buscar botón de siguiente con m├║ltiples selectores mejorados
            next_button_selectors = [
                # Selectores especáficos de Airbnb
                ...
                # Selectores por n├║meros de p├ígina
                ...
                try:
                    ...
                    if btn.count() > 0:
                        ...
                        if first_btn.is_visible():
                            ...
                            print(f"Ô£à Botón de siguiente encontrado con selector: {selector}")
                            break
                except Exception as e:
                    continue
            ...
            if not next_button:
                print("­ƒöì Buscando botón de siguiente con JavaScript...")
                try:
                    # Buscar todos los elementos clickeables que podráan ser de paginación
                    js_result = page.evaluate("""
                        () => {
                            const selectors = [
                                'a[aria-label="Siguiente"]',
                    ...
                    if js_result.get('found'):
                        ...
                        page.wait_for_selector("a[href*='/rooms/']", timeout=15000)
                        pagina += 1
                        print(f"Ô£à Navegación exitosa con JavaScript a la p├ígina {pagina}")
                        time.sleep(2)
                        continue
            ...
            if not next_button:
                print("­ƒöì Intentando navegación directa por URL...")
                try:
                    ...
                    if 'items_offset=' in current_url:
                        ...
                        if match:
                            ...
                            new_offset = current_offset + 20  # Airbnb suele mostrar 20 items por p├ígina
                            new_url = current_url.replace(f'items_offset={current_offset}', f'items_offset={new_offset}')
                            page.goto(new_url)
                            page.wait_for_selector("a[href*='/rooms/']", timeout=15000)
                            pagina += 1
                            print(f"Ô£à Navegación exitosa por URL a la p├ígina {pagina}")
                            time.sleep(2)
                            continue
                except Exception as url_error:
                    print(f"ÔÜá´©Å Navegación por URL falló: {url_error}")
            
            # ESTRATEGIA 4: Buscar botón de siguiente en el contenedor de paginación
            if not next_button:
                print("­ƒöì Buscando en contenedores de paginación...")
                try:
                        ...
                        if container.is_visible():
                            ...
                            for btn in buttons.all():
                                if btn.is_visible():
                                    ...
                                    if any(word in text for word in ['siguiente', 'next', 'ÔåÆ', '>', str(pagina + 1)]):
                                        next_button = btn
                                        print(f"Ô£à Botón encontrado en contenedor: '{text}'")
                                        break
                            if next_button:
                                break
                except Exception as container_error:
                    print(f"ÔÜá´©Å B├║squeda en contenedores falló: {container_error}")
            
            # Si encontramos un botón, hacer clic
            if next_button:
                try:
                    # Hacer scroll al botón para asegurar que estí visible
                    ...
                    # Esperar a que la nueva p├ígina cargue
                    page.wait_for_selector("a[href*='/rooms/']", timeout=15000)
                    
                    # Verificar que realmente cambió de p├ígina
                    current_url = page.url
                    print(f"­ƒöù Nueva URL: {current_url}")
                    
                    pagina += 1
                    print(f"Ô£à Navegación exitosa a la p├ígina {pagina}")
                    time.sleep(2)  # Pausa adicional para estabilizar
                    
                except Exception as click_error:
                    print(f"ÔØî Error haciendo clic en botón siguiente: {click_error}")
                    ...
                    try:
                        ...
                        print(f"ÔØî Navegación con JavaScript falló: {js_error}")
                        print("Ô£à No hay m├ís p├íginas disponibles.")
                        break
            else:
                print("Ô£à No se encontró botón de siguiente p├ígina. Fin del scraping.")
                break
                
        except Exception as e:
            print(f"ÔØî Error general en navegación: {e}")
            print("Ô£à Terminando scraping.")
            break

# Guardar resultado final
print(f"\n Scraping completado!")
print(f"­ƒôè Total de reseñas extraádas: {len(todas_las_reseñas)}")
...
print(f"­ƒÆ¥ Resultado final guardado en: {filename_final}")

...
import { parseDateSpanish, isDateInRange } from "./utils/dateParser"
...
export default function App() {
  ...
  const handleApplyFilters = (filters: FilterValues) => {
    ...
    // Filtrar reseñas primero por fecha y plataforma
    ...
    const filteredNegative = sampleReviews.negative.filter((review) => {
      const matchesPlatform = filters.platforms.includes(review.platform)
      const reviewDate = parseDateSpanish(review.date)
      const matchesDate = isDateInRange(reviewDate, filters.dateFrom, filters.dateTo)
      return matchesPlatform && matchesDate
    ...
    const filteredRecent = sampleReviews.recent.filter((review) => {
      const matchesPlatform = filters.platforms.includes(review.platform)
      const reviewDate = parseDateSpanish(review.date)
      const matchesDate = isDateInRange(reviewDate, filters.dateFrom, filters.dateTo)
      return matchesPlatform && matchesDate
    ...
    // Obtener hoteles únicos de las reseñas filtradas
    const hotelsInFilteredReviews = new Set<string>()
    filteredPositive.forEach(r => hotelsInFilteredReviews.add(r.hotel))
    filteredNegative.forEach(r => hotelsInFilteredReviews.add(r.hotel))
    filteredRecent.forEach(r => hotelsInFilteredReviews.add(r.hotel))
    
    // Filtrar hoteles basándose en las reseñas filtradas
